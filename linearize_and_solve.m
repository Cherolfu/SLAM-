...
for eid = 1:length(g.edges)
  ...
  if (strcmp(edge.type, 'P'))
    ...
    H(j,i) = H(j,i) + B' * edge.information * A;
    H(j,j) = H(j,j) + B' * edge.information * B;
...
%       g.x(edge.fromIdx:edge.fromIdx+2) = 
%       g.x(edge.toIdx:edge.toIdx+2) = x1+x2;
%       g.x(edge.toIdx+2) = normalize_angle(g.x(edge.toIdx+2));
...
%     assignin('base','b',b);
% 
 H(1:3,1:3) = H(1:3,1:3)+0.1*eye(3);

    
      dx = -H\b;
%        dx = lsqr(H,-b,1e-4,100);
